  ...
  <ElementCore :component-id="element.id" @delete="emit('delete', element.id)">
    ...
    <div class="flex items-center gap-2 py-2" v-if="showSettings">
      <div class="flex items-end gap-2">
        ...
        />
        <div class="shrink-0">
          <PrmButton type="button" size="small" icon="pi pi-ellipsis-v" @click="toggleMenu" />
          <PrmMenu ref="menu" size="small" :model="items" :popup="true" />
        </div>
        <div class="w-auto">
          <div>Предупреждение:</div>
          <PrmSelect v-model="selectedWarningAudio" :options="Object.keys(audioMap)" size="small" />
        </div>
        <div>
          <div>Алярма:</div>
          <PrmSelect v-model="selectedAlarmAudio" :options="Object.keys(audioMap)" size="small" />
        </div>
      </div>
    </div>
    <div class="flex gap-2 items-center" v-for="item in formList" :key="item.id">
      ...
      <div v-if="!item.edit && item.startedAt">
        <div v-if="item.finished" style="background-color: red">finished</div>
        <template v-else>
          <PrmInputText
      ...
      <div class="flex flex-col gap-1" v-if="!item.edit">
        <div class="flex gap-1">
          <PrmButton
            class="grow"
            style="padding: 0"
            @click="item.startedAt = dayjs(item.startedAt).add(1, 'seconds').toDate()"
            label="+1"
            size="small"
          />
          <PrmButton
            class="grow"
            style="padding: 0"
            @click="item.startedAt = dayjs(item.startedAt).add(5, 'seconds').toDate()"
            label="+5"
            size="small"
          />
          <PrmButton
            class="grow"
            style="padding: 0"
            @click="item.startedAt = dayjs(item.startedAt).add(10, 'seconds').toDate()"
            label="+10"
            size="small"
          />
        </div>
        <div class="flex gap-1">
          <PrmButton
            class="grow"
            style="padding: 0"
            @click="item.startedAt = dayjs(item.startedAt).subtract(1, 'seconds').toDate()"
            label="-1"
            size="small"
          />
          <PrmButton
            class="grow"
            style="padding: 0"
            @click="item.startedAt = dayjs(item.startedAt).subtract(5, 'seconds').toDate()"
            label="-5"
            size="small"
          />
          <PrmButton
            class="grow"
            style="padding: 0"
            @click="item.startedAt = dayjs(item.startedAt).subtract(10, 'seconds').toDate()"
            label="-10"
            size="small"
          />
        </div>
      ...
      <template v-else>
        ...
        <PrmButton
          ...
          size="small"
        />
        <PrmButton @click="item.edit = true" icon="pi pi-pencil" size="small" />
      </template>
      <PrmButton
...
import type { IElementTimer, TElementID } from '@/type/element';
...
const selectedWarningAudio = shallowRef<keyof typeof audioMap>(
  audioLocalStorage?.warning ?? 'vetsalo_4',
);
const selectedAlarmAudio = shallowRef<keyof typeof audioMap>(
  audioLocalStorage?.alarm ?? 'vetsalo_5_resolve',
);
...
onMounted(() => {
  ...
  intervalID = setInterval(() => {
    ...
    Object.values(formList.value).forEach((_item) => {
      ...
      switch (type) {
        ...
        case 'seconds': {
          ...
          if (seconds < 15 && seconds > 14) {
            // @ts-expect-error: "Object is possibly 'undefined'.ts-plugin(2532)" no it can't
...
watch([selectedWarningAudio, selectedAlarmAudio], ([warning, alarm]) => {
  localStorage.setItem(
    LOCAL_STORAGE_KEY_AUDIO,
    JSON.stringify({
      warning,
      alarm,
    }),
