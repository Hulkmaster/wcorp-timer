...
require$$3$1.ipcMain.on("set-ignore-mouse-events", (event, ignore, options) => {
  const win = require$$3$1.BrowserWindow.fromWebContents(event.sender);
  win?.setIgnoreMouseEvents(ignore, options);
});
require$$3$1.ipcMain.on("minimize", (event) => require$$3$1.BrowserWindow.fromWebContents(event.sender)?.minimize());
require$$3$1.ipcMain.on("close", (event) => require$$3$1.BrowserWindow.fromWebContents(event.sender)?.close());
